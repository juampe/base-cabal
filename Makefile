...
arm64: cache
	$(eval CNAME := $(DOCKER_TAG):$(CABAL_VERSION)-$@)
	docker buildx build $(BUILDX_CACHE) --platform linux/$@ --build-arg CABAL_VERSION=$(CABAL_VERSION) -t $(CNAME) --push .
	docker run --rm $(CNAME) cat /cabal/_build/artifacts/cabal-install-$(CABAL_VERSION)-aarch64-ubuntu-21.04-bootstrapped.tar.xz > repo/cabal-install-$(CABAL_VERSION)-$@-ubuntu-21.04-bootstrapped.tar.xz

riscv64: cache
	$(eval CNAME := $(DOCKER_TAG):$(CABAL_VERSION)-$@)
	docker buildx build $(BUILDX_CACHE) --platform linux/$@ --build-arg CABAL_VERSION=$(CABAL_VERSION) -t $(CNAME) --push .
	docker run --rm $(CNAME) cat /cabal/_build/artifacts/cabal-install-$(CABAL_VERSION)-riscv64-ubuntu-21.04-bootstrapped.tar.xz > repo/cabal-install-$(CABAL_VERSION)-$@-ubuntu-21.04-bootstrapped.tar.xz
